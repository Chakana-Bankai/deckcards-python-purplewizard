...
class AvatarGenerator:
    ...
    def render(self, variant: str, size: tuple[int, int], seed: int = 0) -> pygame.Surface:
        ...
        is_holo = ("oracle" in v) or ("holo" in v)

        cx = w // 2
        ground = int(h * 0.88)

        aura_core = self.PALETTE["archon_red"] if is_archon else self.PALETTE["cyan"]
        aura_ring = self.PALETTE["archon_dark"] if is_archon else self.PALETTE["gold"]
        ...
        robe_main = self.PALETTE["archon_dark"] if is_archon else self.PALETTE["robe"]
        robe_edge = self.PALETTE["archon_red"] if is_archon else self.PALETTE["robe_dark"]
        ...
        skin = (164, 100, 112) if is_archon else self.PALETTE["skin"]
        ...
        beard_col = (96, 58, 110) if is_archon else (110, 88, 132)
        ...
        orb_col = self.PALETTE["archon_red"] if is_archon else self.PALETTE["gold"]
        ...
        emblem_col = self.PALETTE["archon_red"] if is_archon else self.PALETTE["gold"]
