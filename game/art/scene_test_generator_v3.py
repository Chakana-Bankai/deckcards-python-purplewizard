...
def main() -> int:
    ...
    root = project_root()
    out_dir = root / 'assets' / 'art' / 'cards' / 'test_scene_composition_v3'
    report_path = root / 'reports' / 'art' / 'scene_test_v3_metrics.txt'
    out_dir.mkdir(parents=True, exist_ok=True)
    report_path.parent.mkdir(parents=True, exist_ok=True)

    lines = ['scene_test_v3_metrics']
    for index, (label, prompt) in enumerate(SCENE_PROMPTS.items(), start=1):
        out_path = out_dir / f'{label.lower()}_scene_comp_v3.png'
        result = assemble_scene_art(label.lower(), prompt, 6100 + index * 151, out_path)
    ...
    print(f'[scene_test_v3] out={out_dir}')
    print(f'[scene_test_v3] report={report_path}')
    return 0
