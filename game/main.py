...
from game.ui.render import AssetManager, Renderer, SoundManager
...
DEFAULT_LOCALIZATION_KEYS = ["game_title", "menu_play", "menu_exit", "map_title", "button_end_turn", "reward_title", "shop_title"]
DEFAULT_CARDS = [
...
class App:
    ...
    def generate_map(self):
        ...
        by_col = []
        self.node_lookup = {}
        type_cycle = ["combat", "event", "combat", "shop", "combat", "boss"]
        for col in range(columns):
            count = 1 if col in (0, columns - 1) else 2 + (1 if col % 2 == 1 else 0)
            col_nodes = []
            spacing = 360 // max(1, count - 1) if count > 1 else 0
            for row in range(count):
                node_id = f"{col}_{row}"
                y = 180 if count == 1 else 180 + row * spacing
        ...
        for col in range(columns - 1):
            for node in by_col[col]:
                ...
                # guaranteed connectivity: at least one connection, often two
                primary = next_nodes[min(len(next_nodes) - 1, int((node["y"] - 160) / 180) if len(next_nodes) > 1 else 0)]
                node["next"].append(primary["id"])
                if len(next_nodes) > 1:
    ...
    def apply_event_effects(self, effects):
        ...
        for effect in effects:
            ...
            if effect_type == "lose_gold":
                ...
                self.run_state["deck"].append(effect.get("card_id", "strike"))
