...
class CardPreviewPanel:
    ...
    def _icon_row(self, summary: dict):
        ...
        if stats.get("harmony", 0) > 0:
            icon_data.append(("star", stats.get("harmony", 0)))
        ...
        if stats.get("energy", 0) > 0:
            icon_data.append(("bolt", stats.get("energy", 0)))
        return icon_data[:3]
    ...
    def render(self, surface: pygame.Surface, rect: pygame.Rect | None = None, card=None, app=None):
        try:
            ...
            for icon_name, val in icons:
                x_icon = draw_icon_with_value(surface, icon_name, val, self.theme["gold"], body_font, x_icon, y - 2, size=1)
            y += 24
