...
class ShopScreen:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if pygame.Rect(500, 200, 360, 320).collidepoint(pos):
                if self.app.run_state["gold"] >= self.remove_price and self.app.run_state["deck"]:
                    ...
                    self.msg = "No alcanza oro"
            if pygame.Rect(900, 200, 240, 72).collidepoint(pos):
                self.app._complete_current_node()
                self.app.goto_map()
    ...
    def render(self, s):
        ...
        s.blit(self.app.font.render(f"{self.app.loc.t('shop_remove')} ({self.remove_price})", True, UI_THEME["text"]), (518, 220))
        s.blit(self.app.small_font.render("Remueve la primera carta del mazo", True, UI_THEME["muted"]), (518, 260))

        pygame.draw.rect(s, UI_THEME["violet"], (900, 200, 240, 72), border_radius=10)
        s.blit(self.app.font.render(self.app.loc.t("shop_leave"), True, UI_THEME["text"]), (980, 224))
        if self.msg:
