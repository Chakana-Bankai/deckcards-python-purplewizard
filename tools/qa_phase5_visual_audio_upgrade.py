...
def run() -> dict:
    ...
    version_ok = str((man or {}).get("version", "")) in {"chakana_audio_v3", "chakana_audio_v4"}

    portrait_ok = all(x.get("ok", False) for x in portrait_checks.values())
    card_modes_ok = {"procedural_abstract", "procedural_motif", "premium_legendary"}.issubset(set(card_modes))
